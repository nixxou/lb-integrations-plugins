...
// LaunchBox.Metadata.db, where Flycast does not exist while Demul, nullDC and Redream do, all three
// dead for years. Without a row there the grid has nothing to narrow, and the emulator looks like it
...
// checked by name against their database. The day they ship a Flycast row, ours withdraws on its
...
namespace LbIntegrations.NoGba
{
    internal static class LbipRowInjection
    {
        ...
        private static DbConnection EnsureMirror(DbConnection host)
        {
            ...
            // NEVER ADD AN EMULATOR THE HOST ALREADY HAS. The day LaunchBox ships a Flycast row of
            // its own, ours must step aside rather than double it - the user would otherwise get two
            // Flycast entries in the Add Emulator window and no way to tell which is which. Asked of
