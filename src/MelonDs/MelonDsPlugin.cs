...
{
    public partial class MelonDsPlugin : EmulatorPlugin, ISystemEventsPlugin, ILbCatalogSource, IGameLaunchingPlugin
    {
        ...
        internal static bool ChooseConsoleMode(MelonDsLayout layout, string romPath)
        {
            ...
            // belongs to the title that ran last, so launching a plain cartridge must not silently
            // discard the DSiWare session that came before it. With nothing in flight it costs one
            // File.Exists.
            //
            // This used to say there was no "the emulator quit" event to hang the capture on. There
            // is - IGameLaunchingPlugin.OnGameExited - and the plugin now uses it, so most sessions
            // come out of the image as the game closes. This stays all the same: the hook can be
            // missed, a host may not raise it, and a capture that was deferred has to be picked up
        ...
        /// and gives up quickly, because this runs while somebody is watching a frontend come back.
        /// Giving up costs nothing: the lazy path still has the session and a marker beside the
        /// image says so. See DsiWorkspace.CaptureOnExit.
        ///
        /// ON A BACKGROUND THREAD, because the host is calling us and waiting.</summary>
