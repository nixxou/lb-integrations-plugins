// Minimal, surgical reader/writer for ppsspp.ini.
//
// Deliberately NOT a general INI library. It reads named keys out of one section, and writes named
// keys back into that section while preserving every other line — including the comments PPSSPP
...
//  * We refuse to write while PPSSPP is running. PPSSPP rewrites the whole ini from memory when it
...
{
    internal static class PpssppIni
    {
        ...
        public static string Write(string iniPath, string section, IDictionary<string, string> values)
        {
            ...
            if (running != null)
                return "PPSSPP is running (" + running + "). It rewrites its configuration when it exits, "
                     + "which would discard these changes. Close PPSSPP and try again.";
            ...
            try
            {
                ...
                else
                {
                    ...
                    // blank lines, so the file keeps the shape PPSSPP writes.
        ...
        // PPSSPP writes its ini as UTF-8 without a BOM; match that so we don't introduce one.
            ...
            try
            {
                foreach (var p in Process.GetProcesses())
                {
                    using (p)
                    {
                        ...
                        if (n != null && n.StartsWith("PPSSPP", StringComparison.OrdinalIgnoreCase))
                            return n;
                    }
                }
            }
        ...
        /// <summary>PPSSPP writes booleans capitalised ("True"/"False"); match it so a diff of the ini
        /// before and after our edit shows only the values that actually changed.</summary>
        public static string FromBool(bool b) => b ? "True" : "False";
    }
}
